...
honours `--load-extension`, so instead of installing the extension it serves
`extension/` over http and injects a small `chrome` shim before any page
script; only the transport is faked, and the real background handlers run
against the real dictionary in Node. It then drives headless Chrome to check
...
scripts/            data pipeline, icons, shared-lib copy, sync smoke + e2e tests
                    (extension-smoke.mjs loads the real extension in headless
                    Chrome and asserts the popup opens on a hover)
