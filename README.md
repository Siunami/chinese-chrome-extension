...
- **HSK study guides** — a written guide for every level, HSK 1 through 9
  ...
  bundled and works offline, every Chinese character — including Chinese in a
  grammar or mistake title — is hoverable, and
  readings are generated by the same engine that annotates the example corpus
  rather than stored alongside the text. Each level also carries the **complete
  searchable HSK 3.0 vocabulary list**, not just the themed sampler, with
  buttons to practice either that level's new vocabulary or the cumulative set
  through it. **Everything in the written guide is savable**:
  ...
  *sentence* of the reading passage — the passage is offered one sentence at a
  ...
  HSK practice is a separate set and does not add thousands of cards to that
  deck. The schedule belongs to the word, though: if a word is in two HSK sets
  and your saved library, grading it anywhere advances the same schedule
  everywhere. An HSK answer has an explicit **Add to saved library** button.
...
1. Download this repo — **Code → Download ZIP** on GitHub and unzip it, or:
   ...
   The dictionary, example-sentence and HSK vocabulary files are committed, so there is
   nothing to build. (If you ever want to regenerate them from the upstream
   sources, see [Data licenses](#data-licenses). Run
   `node scripts/build-data.mjs` for CC-CEDICT + Tatoeba; after downloading
   `hsk30.csv` from ivankra/hsk30 into `rawdata/`, run
   `node scripts/build-hsk.mjs rawdata/hsk30.csv` for the HSK list.)
...
never call a model. Three details are worth knowing:

- **No readings are stored with the hand-written guide text.** Every Chinese string is annotated
...
- **Chinese in titles is live too.** A grammar title such as “Identity
  sentences with 是” and a mistake title containing 把 or 被 use the same
  phrase-aware hover lookup as an example sentence; they are not inert labels.

### The complete list and HSK practice sets

The themed cards in **Core vocabulary** are a teaching sampler. Below them,
**Complete vocabulary list** contains all 11,092 rows of the 2021 HSK 3.0
standard: 500 at level 1, 772 new at level 2, 973 at level 3, 1,000 at level 4,
1,071 at level 5, 1,140 at level 6, and the shared 5,636-word HSK 7–9 band.
It can switch between the current level and the cumulative list, searches
hanzi, pinyin, part of speech and definitions, and renders 100 rows at a time so
the advanced list does not freeze the guide. `tests/hsk-vocab.test.mjs` pins
every count and checks that all rows have both scripts, pinyin and a back for a
flashcard.

The practice buttons open those lists in the same spaced-repetition review UI
as the saved library, with the same daily limits, grading, interval previews,
pronunciation, examples and tutor. Set membership and memory are separate:

- an HSK word stays out of **Saved library** until **Add to saved library** is
  pressed;
- one card identity has one schedule, even when it belongs to several HSK sets
  and the saved library;
- repeated standard rows such as 白 as an early adjective and a later adverb
  remain visible in the reference list but become one practice card, because
  the spelling and pronunciation point to the same memory;
- shared schedules and HSK-only progress are included in backups. If a word is
  also in the saved library, its schedule continues to sync with that card.

The standard itself groups levels 7–9 into one vocabulary band, so the app says
so and offers one shared set instead of inventing level 7, 8 and 9 splits.
...
extension/          the unpacked extension (load this folder in Chrome)
  ...
  data/             generated: dict.tsv (CC-CEDICT), sentences.tsv (Tatoeba),
                    hsk.tsv (complete 2021 HSK 3.0 vocabulary + CC-CEDICT backs)
  ...
  lib/hsk-vocab.js  complete-list loader, honest 7-9 grouping, set selection
  lib/studysets.js  one review schedule per card identity across HSK sets and
                    the saved library (membership remains separate)
...
scripts/            data pipelines (build-data.mjs + build-hsk.mjs), icons,
                    shared-lib copy, sync smoke + e2e tests
...
- HSK vocabulary identities: the 2021 standard transcription from
  [ivankra/hsk30](https://github.com/ivankra/hsk30), MIT. The readings and most
  definitions added to `hsk.tsv` come from CC-CEDICT under CC BY-SA 4.0. See
  [extension/data/LICENSE.md](extension/data/LICENSE.md) for the combined
